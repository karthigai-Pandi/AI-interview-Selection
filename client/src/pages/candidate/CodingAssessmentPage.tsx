...
const CodingAssessmentPage = () => {
  ...
  if (!workflow.technical.completed) {
    return <Navigate replace to="/candidate/technical" />;
  }
    ...
    <div className="grid gap-6 xl:grid-cols-[0.95fr_0.7fr]">
      <Card title="Coding assessment" description="Choose a language, implement the solution, run tests, and submit your code.">
        <div className="space-y-6">
            ...
            <div className="mt-6 space-y-4 text-slate-300">
              ...
              {problem && (
                <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-4">
                  <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Test cases</p>
          ...
          <div className="rounded-3xl border border-white/10 bg-slate-950/70 p-6">
            <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Code editor</p>
            <textarea
              value={workflow.coding.code}
              onChange={(event) => dispatch(updateCodingCode(event.target.value))}
              className="mt-4 h-[420px] w-full rounded-3xl border border-white/10 bg-slate-900/80 p-4 font-mono text-sm text-slate-100 outline-none focus:border-indigo-400"
            />
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-slate-950/70 p-6 text-slate-200">
            <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Result output</p>
